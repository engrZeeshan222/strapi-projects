...
module.exports = {
  async singleProductScraping(
    ...
    try {
      ...
      let sizeArray = [];
      let size = selector("body")
        .find("#product-description #AddToCartForm .select")
        .text()
        .trim();

      for (let s of sizesList) {
        if (size.toLocaleLowerCase().includes(s.size) && s.size.includes("-")) {
  ...
  async scrapping(brand, colors, fabricList, sizesList, classification) {
    try {
      ...
      selector(collection).each(async (i, element) => {
        ...
        if (link === "https://www.limelight.pk/collections/pret-girls") {
          ...
          link === "https://www.limelight.pk/collections/trouser-girls"
        ) {
          await this.collectionScrapping(
            link,
            brand,
          ...
          link === "https://www.limelight.pk/collections/tights-girls"
        ) {
          await this.collectionScrapping(
            link,
            brand,
